import{a as g,S as X,N as V,P as z,A as J,i as P}from"./assets/vendor-8dsZYxcz.js";(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))o(i);new MutationObserver(i=>{for(const a of i)if(a.type==="childList")for(const u of a.addedNodes)u.tagName==="LINK"&&u.rel==="modulepreload"&&o(u)}).observe(document,{childList:!0,subtree:!0});function s(i){const a={};return i.integrity&&(a.integrity=i.integrity),i.referrerPolicy&&(a.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?a.credentials="include":i.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function o(i){if(i.ep)return;i.ep=!0;const a=s(i);fetch(i.href,a)}})();const n=document.querySelector(".burger-btn"),v=document.querySelector(".nav-wrapper"),c=document.querySelector(".menu-overlay");if(n&&v){const e=()=>{n.classList.add("is-open"),v.classList.add("is-open"),c==null||c.classList.add("is-open"),n.setAttribute("aria-expanded","true"),document.body.style.overflow="hidden"},t=()=>{n.classList.remove("is-open"),v.classList.remove("is-open"),c==null||c.classList.remove("is-open"),n.setAttribute("aria-expanded","false"),document.body.style.overflow=""};n.addEventListener("click",()=>{n.classList.contains("is-open")?t():e()}),v.querySelectorAll(".nav-link, .header-btn").forEach(s=>{s.addEventListener("click",t)}),c==null||c.addEventListener("click",t),document.addEventListener("keydown",s=>{s.key==="Escape"&&n.classList.contains("is-open")&&(t(),n.focus())})}const I=document.querySelector(".pets-section__filter-list"),F=document.querySelector(".pets-section__pets-list"),W=document.querySelector(".pets-section__button"),N=["Собаки","Коти","Кролики","Гризуни","Птахи","Тварини з особливими потребами","Терміново шукають дім"];let l=8,f;const U=e=>{const t=[...e].sort((s,o)=>N.indexOf(s.name)-N.indexOf(o.name)).map(s=>`<li class="filter-list__item">
      ...
      <button class="pets-list__button" type="button" data-id=${s._id} data-description="${s.description}" data-health="${s.healthStatus}" data-behavior="${s.behavior}">Дізнатись більше</button>
    </li>`).join("");G.insertAdjacentHTML("beforeend",t)},oe=async e=>{try{const t=await g.get("https://paw-hut.b.goit.study/api/animals",e);G.innerHTML="",ie(t.data.animals)}catch(t){console.error(t)}},ae=e=>{window.matchMedia("(min-width: 1440px)").matches?x=9:x=8,oe({params:{limit:x}}).catch(t=>{console.log(t)})};document.addEventListener("DOMContentLoaded",ae);const re=document.querySelector(".pets-section__pets-list"),A=document.querySelector(".pets-section__button");let m=document.querySelector(".filter-list__button.is-active"),p=8,h=m.dataset.id,d=1,C,D=500;const ne=e=>{const t=[...e].map(s=>`<li class="pets-list__item">
      <img src="${s.image}" alt="${s.name}" class="pets-list__image" />
      <p class="pets-list__species">${s.species}</p>
      <p class="pets-list__name">${s.name}</p>
  ...
  `}async function me(){const e=await de(),t=document.querySelector("#feedbacks-list");t.innerHTML=e.map(ue).join("")}document.addEventListener("DOMContentLoaded",async()=>{await me(),new X(".swiper__success-swiper",{modules:[V,z],slidesPerView:1,slidesPerGroup:1,spaceBetween:20,navigation:{nextEl:".success-swiper__button-next",prevEl:".success-swiper__button-prev"},pagination:{el:".success-swiper__pagination",clickable:!0},breakpoints:{768:{slidesPerView:2,slidesPerGroup:2,spaceBetween:32}}})});const _e=document.querySelector(".modal-overlay"),he=document.querySelector(".pets-section__pets-list"),j=document.querySelector("body"),R=document.querySelector(".animal-modal"),b=document.querySelector(".animal-modal-backdrop"),fe=document.querySelector(".modal-button");let r,K;const M=()=>{j.classList.remove("no-scroll"),b.classList.remove("is-open"),setTimeout(()=>{R.innerHTML=""},250)},ge=()=>{const e=()=>`<button class="animal-modal__animal-close-button is-animal-close-button">
    ...
    </div>`;R.insertAdjacentHTML("beforeend",e())},be=e=>{e.target.classList.contains("pets-list__button")&&(K=e.target.dataset.id,b.classList.add("is-open"),j.classList.add("no-scroll"),r=e.target.closest(".pets-list__item"),ge())},ve=e=>{e.target.classList.contains("is-animal-close-button")&&M()},ye=e=>{e.target.classList.contains("take-home-btn")&&(_e.classList.add("is-open"),M(),j.classList.add("no-scroll"),fe.dataset.id=K)};he.addEventListener("click",be);b.addEventListener("click",ve);b.addEventListener("click",ye);window.addEventListener("keydown",e=>{e.key==="Escape"&&M()});b.addEventListener("click",e=>{e.target===e.currentTarget&&M()});const Q=document.querySelector(".modal-overlay"),we=document.querySelector("[data-order-modal-close]"),Le=document.querySelector(".modal-form"),S=document.querySelector('[name="user-name"]'),y=S.nextElementSibling,k=document.querySelector('[name="user-phone"]'),w=k.nextElementSibling,q=document.querySelector('[name="user-comment"]'),L=q.nextElementSibling,$e=document.querySelector(".modal-button");let T,H,B;const Se="https://paw-hut.b.goit.study/api/orders";function E(){Q.classList.remove("is-open"),document.body.classList.remove("no-scroll")}we.addEventListener("click",E);window.addEventListener("keydown",e=>{e.key==="Escape"&&E()});Q.addEventListener("click",e=>{e.target===e.currentTarget&&E()});Le.addEventListener("submit",async e=>{e.preventDefault();const t=S.value.trim(),s=k.value.trim();let o=q.value.trim();o===""&&(o="коментар відсутній");const i=$e.dataset.id;if(ke(t),qe(s),Me(o),T&&H&&B){const a={name:t,phone:s,animalId:i,comment:o};try{const u=await g.post(Se,a);P.success({title:"",message:`Ваше замовлення успішно збережено під номером ${u.data.orderNum}`}),E()}catch{P.error({title:"",message:"Вибачте сталася помилка при надсиланні запиту"})}finally{e.target.reset()}}});function ke(e){const t=s=>{y.classList.add("show-error"),S.classList.add("error-border"),y.innerHTML=`${s}`,T=!1};e===""?t(`Поле "Ім'я" обов'язкове до заповнення`):e.length>32?t("Ім'я не повинно перевищувати 32 символи"):/\d/.test(e)?t("Ім'я не повинно містити цифр"):(y.classList.remove("show-error"),S.classList.remove("error-border"),y.innerHTML="",T=!0)}function qe(e){/^380\d{9}$/.test(e)?(w.classList.remove("show-error"),k.classList.remove("error-border"),w.innerHTML="",H=!0):(w.classList.add("show-error"),k.classList.add("error-border"),w.innerHTML="Введіть телефон у форматі 380XXXXXXXXX",H=!1)}function Me(e){e.length>500?(L.classList.add("show-error"),q.classList.add("error-border"),L.innerHTML="Коментар не повинен перевищувати 500 символів",B=!1):(L.classList.remove("show-error"),q.classList.remove("error-border"),L.innerHTML="",B=!0)}document.querySelector("#global-loader");
